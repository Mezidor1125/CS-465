...
// required variable that parses the contents of the file and inserts them into the index variable (SNHU, 2023, p. 1)
var admins = JSON.parse(fs.readFileSync('./data/admin.json', 'utf8'));

/* GET admin view (SNHU, 2023, p. 1) */

// const admin variable that is used to set the declared function which takes the paramters req and res (SNHU, 2023, p. 1)
const admin = (req, res) => {
    
    // function used to render view and send the rendered HTML string to the client, meaning sends the title Travlr Getaways to the client denoted by admin (GeeksForGeeks, 2023, p. 1);(SNHU, 2023, p. 1)
    res.render('admin', {title: 'Travlr Getaways | Admin', admins });
};

// object in the Node.js file that holds the exported values and functions from that module, in the case of it being the module exporting to the admin variable (Megida, 2022, p. 1);(SNHU, 2023, p. 1)
module.exports = {
    admin
};